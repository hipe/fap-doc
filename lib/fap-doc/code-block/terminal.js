...
exports.term2sexp = function(str) {
  ...
  function match() { // like so just for debuggin
    return exports.regexp.exec(str);
  }
  while ((md = match())) {
...
debugger;
    if ('' == lines[lines.length-1]) lines.pop();
      // Most terminal output has a single trailing newline, which was turned
      // into an empty trailing string as the last item of the above array.
      // We don't want it because it looks ugly creating a widow line in the
      // output so we strip off one of those.  Change original meh.
  ...
  render : function() {
    ...
    var addHere = ts.getElementsByTagName("pre")[0];
  ...
  _buildEmptyTagset : function() {
    // <div class="{__cssClassStr}"><code><pre>..</pre></code></div>
    var doc = this._document(), el1, el2, el3;
    (el1 = doc.createElement('div')).appendChild(
      el2 = doc.createElement('code'));
    el2.appendChild(el3 = doc.createElement('pre'));
    el1.setAttribute('class', this.__cssClassStr);
    return el1;
  },
  _document : function() {
