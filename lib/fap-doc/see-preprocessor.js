...
SeeStory.prototype = extend({}, SeeCommon, {
  __token : 'story',
  render : function (nonwhite, white) {
    ...
    var indent = (!nonwhite.length && white.length) ? white : '';
...
/**
 * Given a string that is the filepath and a string that is the story name,
 * run the test (isomorphic with story name, i.e. the same!) and for now
 * just grab lastOutput!
 *
 */
...
SeeOutput.prototype = extend({}, SeeCommon, {
  ...
  render : function (nonwhite, white) {
    if (!this._prerender()) return this._unparsed;
    var me = this, fu = require(require('./paths').fapUnit+'/lib/fap-unit');
    var tc = fu.suppressRun(function(){ return require(me._filename); });
    if (0 == (new fu.Runner()).addLiteralTestFilter(this.__storyName).
      addCase(tc).disableAnnouncements().run().numTestsRan()
    ) {
      this._warn('couldn\'t find "' + this.__storyName +'" in '+this._filename);
      return this._unparsed;
    }
    var indent = (!nonwhite.length && white.length) ? white : '';
    debugger
    return util.reindent(tc.lastOutput, '', indent);
  }
});
