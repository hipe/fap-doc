...
var _CodeBlock = {
  ...
  */
  factory : function(chunkIdx, blockIdx, lines, cbm) {
    ...
    var type = cbm.propertyMeta(blockIdx, 'type'), codeBlock;

    if (claz.found && claz.proximity < type.proximity) {
      codeBlock = new CssStyledCodeBlock(chunkIdx, blockIdx, lines, cbm,
        claz.value);
    } else if ('none' == type.value) {
...
var CssStyledCodeBlock = function() {
  this._codeBlockInit.apply(this, arguments);
  this.__meta = this._buildMeta();
};

CssStyledCodeBlock.prototype = extend({}, _CodeBlock.proto, {
