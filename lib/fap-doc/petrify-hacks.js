 ...
 */
var withFileList = exports.withFileList = function(dirname, list, fn, callback){
...
exports.BuildrunnerHacks = {
  ...
  _hackDataFiles : function() {
    ...
    this._petrify._hacks._loadDataFilesHack();
    this._dataFilesHacked = true;
  }
};


var PetrifyHacks = exports.PetrifyHacks = function(petrify) {
  this._petrify = petrify;
  this._metadata = {};
};
PetrifyHacks.prototype = {
  ...
  setMetadata : function(path, md) {
    this._metadata[path] = md;
  }
};

/**
 * ************ begin code-blocks hack ***************************
 */
exports.prepareCodeBlockProcessingForPage = function(pageData) {
  var h = (exports.__libs && exports.__libs.petrify &&
    exports.__libs.petrify._hacks);
  if (!h) {
    sys.logs("hacks error: cannot prepareCodeBlockProcessingForPage() without "+
      "calling petHacks.enableHacks(petrify,..) first");
    return false;
  }
  return h._prepareCodeBlockProcessingForPage(pageData);
};
PetrifyHacks.prototype._prepareCodeBlockProcessingForPage = function(pageData){
  sys.log("OHAI SKIPPING THIS WHATEVER");
};
// ************ end hack ************
 ...
 *
 */
PetrifyHacks.prototype._loadDataFilesHack = function() {
  if (this.__loadDataFilesHacked) return null;
  ...
  var origLoadData = this._petrify.loadData;
  this._petrify.loadData = function(dirname) {
  ...
  exports.PetrifyHacks.prototype.addCrazyExtraDataFiles =
    ...
    var h = this;
    withFileList(dirname, (this._strangeDataFiles || []),
      function(filename, fileData, completed, total){
        datas.push(h._petrify.readFile(filename, fileData));
  ...
  var prevReadFile = this._petrify.readFile, hacks = this;

  this._petrify.readFile = function(filename, data) {
    var result = prevReadFile(filename, data), i;
    result.filename = filename; // overwrite the basename() version of it
    if (hacks._metadata[filename]) {
      for (i in hacks._metadata[filename]) {
        result.metadata[i] = hacks._metadata[filename][i];
      }
    }
    return result;
  };
  ...
  var origEmit = this._petrify.emit;
  this._petrify.emit = function(output_dir, urlpath, data, callback) {
  ...
  this.__loadDataFilesHacked = true;
  return true;
};
