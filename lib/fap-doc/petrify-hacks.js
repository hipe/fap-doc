...
exports.hackPetrify = function(libs, run, f) {
  h = new BuildrunnerHacks(libs, run);
  if (f) f(h);
...
var BuildrunnerHacks = function(libs, buildRunnerLib) {
  ...
  // run @todo ask caolan!)
  if (!buildRunnerLib.__hacked) {
    buildRunnerLib.__hacked = true;
    var r = buildRunnerLib.run, hax = this;
    buildRunnerLib.run = function(options){
      var runner = r.call(buildRunnerLib, options);
      hax.__runner = runner;
      return runner;
    };
  }
