var events = require ('events'),
      ...
      Path = require('path'),
     paths = require('./paths'),
     color = require(paths.optparse + '/lib/fuckparse').color.methods.color;
...
BuildrunnerHacks.prototype = {
  ...
  processSidebars : function() {
    if (this.__sb) return; this.__sb = true;
    if (!this.__hackedReadFile) this._hackReadFile();
    var pp = require('./sidebar-postprocessor').build(this);
    // things get borky if we run this after html_no_heading wmd postprocessor
    this.__processors.post.unshift(function(d) { return pp.process(d); });
  },
  processSee : function() {
    if (this.__see) return; this.__see = true;
    if (!this.__hackedReadFile) this._hackReadFile();
    var pp = require('./see-preprocessor').build(this);
    this.__processors.pre.unshift(function(d) { return pp.process(d); });
  },
  _color : color,
